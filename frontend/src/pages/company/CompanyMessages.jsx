...
import { Send, MessageSquare, Wifi, WifiOff, User, Lock, ArrowLeft } from 'lucide-react';
...
export default function CompanyMessages() {
  ...
  useEffect(() => {
    api.get('/company/chats').then(res => {
      setRooms(res.data);
  ...
  // Mobile-only: return to the candidate list without losing socket state
  ...
  const studentName = (room) =>
    `${room.Student?.firstName || ''} ${room.Student?.lastName || ''}`.trim() || 'Student';
  const avatarLetter = (room) => room.Student?.firstName?.[0]?.toUpperCase() || 'S';
  ...
  return (
    <CompanyLayout>
      ...
      <div
        ...
        {/* Candidate list — full-width on mobile, hides once a chat is opened */}
        <div
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
            {rooms.length === 0 ? (
              <div style={{ padding: '24px 12px', textAlign: 'center' }}>
                <User size={24} style={{ color: c.border, margin: '0 auto 8px' }} />
                <p style={{ fontSize: '12px', color: c.txt3, lineHeight: '1.5', margin: 0 }}>
                  No conversations yet.<br />Accept an applicant to unlock messaging.
                </p>
              </div>
            ) : rooms.map(room => {
              ...
              return (
                <button key={room.id} onClick={() => joinRoom(room)} style={{
                  ...
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <div style={{ width: '32px', height: '32px', background: isActive ? 'rgba(255,255,255,0.2)' : c.surface2, border: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '700', color: isActive ? '#fff' : c.red, borderRadius: '50%' }}>
                      {avatarLetter(room)}
                    </div>
                    {badge > 0 && (
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    {room.Student?.degreeProgram && (
                      <p style={{ fontSize: '10px', color: isActive ? 'rgba(255,255,255,0.7)' : c.txt3, margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {room.Student.degreeProgram}
                      </p>
                    )}
                    {room.isLocked && (
        ...
        <div className={`card ${!activeRoom ? 'max-md:hidden' : ''}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {!activeRoom ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', color: c.txt3 }}>
              ...
              <p style={{ fontSize: '14px', fontWeight: '500', margin: 0 }}>Select a candidate</p>
              <p style={{ fontSize: '12px', color: c.txt3, margin: 0 }}>Choose a student from the list to start chatting</p>
            </div>
          ) : (
            <>
              {/* Header */}
              <div style={{ padding: '12px 16px', borderBottom: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', gap: '10px', background: c.surface }}>
                ...
                <div style={{ width: '36px', height: '36px', background: c.red, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '700', fontSize: '14px', flexShrink: 0 }}>
                  {avatarLetter(activeRoom)}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '14px', fontWeight: '700', color: c.txt1, margin: 0 }}>{studentName(activeRoom)}</p>
                  {isLocked ? (
                  ...
                  ) : activeRoom.Student?.degreeProgram ? (
                    <p style={{ fontSize: '11px', color: c.txt3, margin: '2px 0 0' }}>{activeRoom.Student.degreeProgram}</p>
                  ) : null}
                </div>
              </div>
              ...
              {isLocked && (
                <div style={{ padding: '12px 16px', background: 'rgba(239,68,68,0.06)', borderBottom: `1px solid rgba(239,68,68,0.2)`, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <p style={{ fontSize: '12px', color: '#EF4444', margin: 0, fontWeight: '500' }}>
                    This application was rejected. The conversation is now read-only.
                  </p>
                </div>
              )}
