...
export default function CompanyLayout({ children }) {
  ...
  useEffect(() => {
    api.get('/company/unread-count')
      .then(res => setUnreadCount(res.data.count || 0))
      .catch(() => {});
    api.get('/company/notifications/count')
      .then(res => setNewApplicantCount(res.data.count || 0))
      .catch(() => {});
  }, []);

  const NAV = [
    { to: '/company/dashboard',    icon: LayoutDashboard, label: 'Dashboard'       },
    { to: '/company/jobs',         icon: FilePlus,        label: 'Manage Jobs'     },
    { to: '/company/applications', icon: Users,           label: 'Applications', badge: newApplicantCount },
    { to: '/company/messages',     icon: MessageSquare,   label: 'Messages', badge: unreadCount },
  ...
  const Sidebar = () => (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: c.surface, borderRight: `1px solid ${c.border}` }}>
      ...
      <div style={{ padding: '14px 20px', borderBottom: `1px solid ${c.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '34px', height: '34px', background: avatarSrc ? 'transparent' : '#1A2235', borderRadius: '50%', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {avatarSrc
          ...
          <div style={{ overflow: 'hidden' }}>
            ...
            <p style={{ fontSize: '11px', color: c.red, margin: '3px 0 0', fontWeight: '500' }}>HR Partner</p>
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: '10px 8px' }}>
        ...
        {NAV.map(({ to, icon: Icon, label, badge }) => (
          <NavLink key={to} to={to} onClick={() => {
            ...
            if (to.includes('applications') && newApplicantCount > 0) {
              setNewApplicantCount(0);
              api.post('/company/notifications/mark-seen').catch(() => {});
            }
          }}
      ...
      <nav className="bottom-nav" style={{ display: 'none' }}>
        {NAV.map(({ to, icon: Icon, label, badge }) => (
          <NavLink key={to} to={to} onClick={() => {
            ...
            <Icon size={20} />
            <span>{label === 'Manage Jobs' ? 'Jobs' : label === 'Company Profile' ? 'Profile' : label}</span>
